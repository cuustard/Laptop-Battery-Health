...
function formatDateTime(
  ...
  const formatted = `${getOrdinalDay(parsed.getDate())} ${parsed.toLocaleString(
    undefined,
    {
      month: "long",
    }
...
function App() {
  ...
  const [isSavingManagedSettings, setIsSavingManagedSettings] = useState(false);
  const [lastSnapshotMessage, setLastSnapshotMessage] = useState<string | null>(
    null
  );
  ...
  useEffect(() => {
    ...
    document.documentElement.dataset.textSize = preferences.largeText
      ? "large"
      : "standard";
    document.documentElement.dataset.palette = preferences.colorBlindFriendly
      ? "colorblind"
      : "default";
  }, [preferences]);
  ...
  async function loadDesktopSettings() {
    try {
      ...
      setSettingsSaveMessage(
        err instanceof Error ? err.message : "Failed to load desktop settings."
      );
    }
  }
  ...
  async function refreshHistory() {
    try {
      const loaded = await invoke<BatterySnapshot[]>("load_battery_history");
      setHistory(loaded);
    } catch (err) {
  ...
  function handleExportSummary() {
    ...
    const lines = [
      ...
      `Full charge capacity: ${formatMWh(battery?.fullChargeCapacity_mWh)}`,
      `Design capacity: ${formatMWh(battery?.designCapacity_mWh)}`,
      `Cycle count: ${
        battery?.cycleCount !== undefined && battery?.cycleCount !== null
          ? battery.cycleCount
          : "N/A"
      }`,
      "",
        ...
        <header className="app-header">
          <div className="app-header__content">
            ...
            <p className="app-subtitle">
              View battery health, wear, capacity, and device details from your
              ...
              <div className="settings-group">
                <label className="settings-toggle">
                  ...
                  <span>
                    ...
                    <small>
                      Save a snapshot automatically when a report is loaded.
                    </small>
                  </span>
                </label>
              ...
              <div className="settings-group">
                ...
                <label className="settings-toggle">
                  ...
                  <span>
                    ...
                    <small>
                      Check battery health on a schedule even while the app is
                      hidden in the tray.
                    </small>
                  </span>
                </label>
              ...
              <div className="settings-save-bar">
                ...
                <div className="settings-save-bar__actions">
                  <button
                    ...
                    disabled={
                      !hasPendingSettingsChanges || isSavingManagedSettings
                    }
                  >
                    Reset
                  </button>
                  ...
                  >
                    {isSavingManagedSettings ? "Saving..." : "Save changes"}
                  </button>
                </div>
              </div>
            ...
            {data ? (
              <div className="details-grid">
                  ...
                  {hasMetadata ? (
                    <>
                      ...
                      <InfoRow
                        ...
                        value={data.metadata.systemProductName ?? "N/A"}
                      />
                      <InfoRow
                        label="BIOS"
                      ...
                      <InfoRow
                        label="Report time"
                        value={formatDateTime(data.metadata.reportTime)}
                      />
                    </>
                  ) : (
                ...
                >
                  {battery ? (
                    <>
                      <InfoRow label="Name" value={battery.name ?? "N/A"} />
                      <InfoRow
                        label="Manufacturer"
                      ...
                      <InfoRow
                        ...
                        value={formatMWh(battery.designCapacity_mWh)}
                      />
                      <InfoRow
                        label="Full charge capacity"
                        value={formatMWh(battery.fullChargeCapacity_mWh)}
                      />
                    </>
                  ) : (
                    <p className="app-empty-state">
          ...
          <>
            ...
            {!data && !error && !isLoading && (
              <section className="status-card">
                <h2 className="status-card__title">No battery report loaded</h2>
                <p className="status-card__text">
                  Click <strong>Load Battery Report</strong> to parse your
                  Windows battery report and view battery health, capacity
                  history, recent usage, and insights.
                </p>
              </section>
            )}
            ...
            {!data && isLoading && (
              <section className="status-card">
                <h2 className="status-card__title">Loading battery report</h2>
                <p className="status-card__text">
            ...
            {lastSnapshotMessage && (
              <section className="status-card">
                <h2 className="status-card__title">Snapshot status</h2>
                <p className="status-card__text">{lastSnapshotMessage}</p>
              </section>
            )}
            ...
            {data && (
              <div className="dashboard-stack">
                  ...
                  {comparison.previous && comparison.latest ? (
                    <div className="details-grid">
                      <div>
                        <InfoRow
                          label="Health change"
                          value={formatDelta(comparison.healthDelta, "%")}
                        />
                        <InfoRow
                          label="Wear change"
                          value={formatDelta(comparison.wearDelta, "%")}
                        />
                        <InfoRow
                          label="Health trend"
                      ...
                      <div>
                        ...
                        <InfoRow
                          ...
                          value={formatDelta(comparison.cycleCountDelta, "", 0)}
                        />
                        <InfoRow
                    ...
                    <p className="app-empty-state">
                      Load at least two distinct reports on this device to see
                      change over time.
                    </p>
                  )}
                </SectionCard>
                ...
                {battery ? (
                  <section aria-label="Battery overview" className="stats-grid">
                    <StatCard
                  ...
                  <section className="status-card status-card--warning">
                    ...
                    <p className="status-card__text">
                      The report loaded, but Windows did not include any
                      installed battery entries.
                    </p>
                  </section>
                )}
                ...
                >
                  {insights.length > 0 ? (
                    ...
                    <p className="app-empty-state">
                      No insights could be generated from this report.
                    </p>
                  )}
                </SectionCard>
                ...
                >
                  {hasCapacityHistory ? (
                    <CapacityHistoryChart data={data.capacityHistory} />
                  ) : (
                    <p className="app-empty-state">
                      No capacity history was found in this report.
                    </p>
                  )}
                </SectionCard>
                ...
                >
                  <div className="usage-note">
                    <p className="usage-note__intro">
                      This chart shows short-term battery behavior over recent
                      sessions, not long-term health.
                    </p>

                    <ul className="usage-note__list">
                      <li>
                        <strong>Battery level (%)</strong> shows how charge
                        changes over time.
                      </li>
                      <li>
                        <strong>Steep drops</strong> indicate heavy usage or
                        power-intensive activity.
                      </li>
                      <li>
                        <strong>Flat sections</strong> often mean the device was
                        idle or in standby.
                      </li>
                      <li>
                        <strong>Charging periods</strong> can be seen where the
                        line increases.
                      </li>
                    </ul>

                    <p className="usage-note__footer">
                      Data is based on recent system activity logs and may not
                      be continuous.
                    </p>
                  </div>
                  ...
                  {hasRecentUsage ? (
                    ...
                    <p className="app-empty-state">
                      No recent usage entries were found in this report.
                    </p>
                  )}
                </SectionCard>
              </div>
